######################################################################
#Analysis of Dobnikar et al data
######################################################################
...
library(reticulate)
library(cerebroApp)
setwd("~/Documents/Bioinfo/Ebru/Dobnikar/")

#Load data
lin_pos <- Read10X_h5('data/GSE117963_10X_lineage_positive_filtered_gene_bc_matrices_h5.h5')
hfd14w <- Read10X_h5('data/GSE117963_10X_plaque_14w_filtered_gene_bc_matrices_h5.h5')
hfd18w <- Read10X_h5('data/GSE117963_10X_plaque_18w_filtered_gene_bc_matrices_h5.h5')
whole <- Read10X_h5('data/GSE117963_10X_whole_aorta_filtered_gene_bc_matrices_h5.h5')

lin_pos <- CreateSeuratObject(lin_pos,  project = 'Myh11-CreERt2+ ApoE-/-')
hfd14w <- CreateSeuratObject(hfd14w, project = 'Myh11-CreERt2+ ApoE-/- HFD 14w')
hfd18w <- CreateSeuratObject(hfd18w, project = 'Myh11-CreERt2+ ApoE-/- HFD 18w')
whole <- CreateSeuratObject(whole, project = 'Whole Aorta')

######################################################################
#QC and filtering 
######################################################################
dats <- list(lin_pos, hfd14w, hfd18w, whole)
...
# Lineage-positive control, medial layer
dats[[1]] <- subset(dats[[1]], subset = nFeature_RNA > 300 & nFeature_RNA < 2500 &
                      nCount_RNA < 8000 & percent.mt < 25)
# Myh11-CreERt2+ ApoE-/- HFD 14w
dats[[2]] <- subset(dats[[2]], subset = nFeature_RNA > 500 & nFeature_RNA < 4000 &
                      nCount_RNA < 15000 & percent.mt < 25)
# Myh11-CreERt2+ ApoE-/- HFD 18w
dats[[3]] <- subset(dats[[3]], subset = nFeature_RNA > 500 & nFeature_RNA < 3500 &
                      nCount_RNA < 15000 & percent.mt < 25)
# Whole aorta
dats[[4]] <- subset(dats[[4]], subset = nFeature_RNA > 300 & nFeature_RNA < 4000 &
                      nCount_RNA < 15000 & percent.mt < 25)

#Add some metadata
l_linpos <- rep('Lineage-positive control, medial layer', times = ncol(dats[[1]]))
l_hfd14 <- rep('Lineage positive ApoeKO 14w HFD', times = ncol(dats[[2]]))
l_hfd18 <- rep('Lineage positive ApoeKO 18w HFD', times = ncol(dats[[3]]))
l_whole <- rep('Whole aorta', times = ncol(dats[[4]]))
label <- c(l_linpos, l_hfd14, l_hfd18, l_whole)
...
FeaturePlot(dat, features = 'Cd209a', order = T, min.cutoff = 0, max.cutoff = 1, pt.size = 1)
...
DimPlot(dat, reduction = 'dbmap', group.by = 'pred.kal', label = T, label.size = 5, repel = T, pt.size = 1)
...
# Save RDS
saveRDS(dat, 'Dobnikar.Rds')

# Convert to h5Seurat, h5ad
library(SeuratDisk)
as.h5Seurat(dat, 'Dobnikar.h5Seurat', overwrite = T)
Convert('Dobnikar.h5Seurat', dest = 'h5ad', overwrite = T, assay = 'RNA')
...
dat <- getMarkerGenes(dat, organism = 'mm', groups = c('pca_clusters', 'gene_clusters', 'seurat_clusters', 'pred.kal', 'Sample', 'Phase'), min_pct = 0.3, assay = 'integrated', test = 'wilcox')
...
dat <- performGeneSetEnrichmentAnalysis(dat, assay = 'integrated', 
                                        groups = c('pca_clusters', 'gene_clusters', 'seurat_clusters', 'pred.kal', 'Sample', 'Phase'),
...
cerebro <- exportFromSeurat(dat, file = 'Dobnikar.crb', experiment_name = 'Dobnikar et al', organism = 'mm',
                            cell_cycle = 'Phase', groups = c('pca_clusters', 'gene_clusters', 'seurat_clusters', 'pred.kal','Sample', 'Phase'),
