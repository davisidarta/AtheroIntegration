...
library(reticulate)
library(cerebroApp)
setwd("~/Documents/Bioinfo/Vascular/Kim/")

#Load data
c <- Read10X('data/Total', gene.column = 2)
h <- Read10X('data/LDLR_KO', gene.column = 2)
...
c <- subset(c, subset = nFeature_RNA > 2000 & nFeature_RNA < 8000 &
              nCount_RNA < 50000 & percent.mt < 15)

# High-fat diet (atherosclerosis progression)
...
h <- subset(h, subset = nFeature_RNA > 1000 & nFeature_RNA < 5000 &
...
for(i in 1:length(sets)){
  ...
  sets[[i]] <- FindVariableFeatures(sets[[i]], selection.method = 'disp', nfeatures = 1000)
...
dat <- RunUMAP(dat, reduction = 'sc', dims = 1:ncol(dat@reductions$sc@cell.embeddings), n.neighbors = 10, init = 'spectral',
               min.dist = 0.6, spread = 1.5, learning.rate = 1.5, n.epochs = 800, reduction.key = 'dbMAP_', reduction.name = 'dbmap')
...
library(SeuratDisk)
as.h5Seurat(dat, 'Kim.h5Seurat', overwrite = T)
Convert('Kim.h5Seurat', dest = 'h5ad', overwrite = T, assay = 'RNA')
...
dat <- getMarkerGenes(dat, organism = 'mm', groups = c('pca_clusters', 'gene_clusters', 'seurat_clusters', 'Group', 'Phase'), min_pct = 0.3, assay = 'integrated', test = 'wilcox')
...
cerebro <- exportFromSeurat(dat, file = 'Sharma.crb', experiment_name = 'Sharma et al', organism = 'mm',
                            cell_cycle = 'Phase', groups = c('pca_clusters', 'seurat_clusters', 'Group', 'Phase'),
